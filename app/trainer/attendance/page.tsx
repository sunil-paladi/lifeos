...
export default async function TrainerAttendancePage() {
  ...
  const memberships = await getAttendanceMembershipOptions(user.id, "TRAINER");

  if (memberships.length === 0) {
    return (
      <section className="rounded-xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
        <h1 className="font-semibold">Client attendance unavailable</h1>
        <p className="mt-1 text-sm">You do not have an active trainer membership.</p>
      </section>
    );
  }

  return <AttendanceView role="TRAINER" memberships={memberships} />;
}